...
def apply_paths(char, target_level, build_config, settings):
    ...
    for i, pconf in enumerate(path_list):
        ...
        # Whole levels first (required), then non-repeatable sub-levels (optional)
        ...
        remaining = share - achievements
        # If this path has leftover STP, take more of repeatables already consumed
        if remaining > 0 and repeatables:
            for key in repeat_keys:
                max_count = repeatables.get(key, 0)
                already = repeat_taken_global.get((path_name, arch_name, key), 0)
...
def spell_avg_damage(spell, element, aff_val, die_avg, hit_chance, char=None, weapon_info=None):
    ...
    # Add Base Magic Damage from character (Magician 3.5, etc.)
    if char:
        dmg += getattr(char, 'magic_damage', 0)

        # Add element-specific damage bonus (e.g. Pyromancer +1d4 Fire)
...
def calculate_10_round_damage(char, r, dmg_per_turn, settings):
    ...
    # Magical path — include first-round bonus on spell casts
...
def write_summary(all_tier_results, settings, output_path, build_configs=None):
    ...
    # Ranged mobility bonus: builds with ranged attacks don't waste turns moving
    RANGE_BONUS = 15
