...
set_base_url() {
  ...
  if grep -qE 'baseUrl:\s*["'\'']' "$CFG"; then
    # Portable in-place replace; creates and removes .bak on both macOS and Linux
    sed -E -i.bak "s|baseUrl:[[:space:]]*['\"][^'\"]*['\"]|baseUrl: \"$BASE_URL\"|g" "$CFG"
    rm -f "$CFG.bak"
  else
    # Insert after the first occurrence of "export default defineConfig({"
    awk -v base="$BASE_URL" '
      BEGIN{ inserted=0 }
      {
        print
        if (!inserted && $0 ~ /export[[:space:]]+default[[:space:]]+defineConfig\(\{/) {
          print "  baseUrl: \"" base "\","
          inserted=1
        }
      }
...
build_site() {
  echo "Building site..."
  pushd "$SITE_DIR" >/dev/null
    if need_cmd jq && jq -re '.scripts.build' package.json >/dev/null 2>&1; then
      npm run build
    else
      npx quartz build
    fi
...
serve_site() {
  ...
  pushd "$SITE_DIR" >/dev/null
    if need_cmd jq && jq -re '.scripts["build:serve"]' package.json >/dev/null 2>&1; then
      npm run build:serve
    else
